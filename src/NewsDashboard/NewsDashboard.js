...
const NewsDashboard = () => {
  ...
  const fetchNews = async () => {
    const res = await axios.get('https://backend-co2e-portal.vercel.app/api/news');
    setNews(res.data);
  };
  ...
  const handleSubmit = async e => {
    ...
    await axios.post('https://backend-co2e-portal.vercel.app/api/news/add', data);
    fetchNews();
    setForm({ 
  ...
  const handleDelete = async (id) => {
    await axios.delete(`https://backend-co2e-portal.vercel.app/api/news/${id}`);
    fetchNews();
  };
